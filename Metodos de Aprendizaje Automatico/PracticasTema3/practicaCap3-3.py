...
# con el formato de la práctica anterior todo junto en este DataFrame.
# frequent_itemsets = apriori(onehot_df, min_support=0.0005, max_len=3, use_colnames=True)

# 3. Comprobar el soporte mínimo de entre todas estas reglas y ponerlo en un comentario.
# El soporte minimo es de 0.001189, para los items (Thriller, Sci-Fi, War)
...
# 7. A partir del DataFrame generado por association_rules, generar un DataFrame que 
# tenga el formato que hemos seguido en las prácticas de la semana pasada. 
# Sólo queremos "rule", "support", "confidence", "lift".
# rules['support'] rules['confidence'] rules['lift']
new_df = pd.DataFrame({
    'rule': rules['antecedents'].apply(lambda x: ', '.join(x)) + ' -> ' + rules['consequents'].apply(lambda x: ', '.join(x)),
